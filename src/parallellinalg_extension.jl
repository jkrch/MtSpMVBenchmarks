...
function mul!(y::StridedVector, Trans::Transpose{<:Any,<:AbstractSparseMatrixCSC}, x::StridedVector, α::Number, β::Number) where  
    Trans.n == length(x) || throw(DimensionMismatch("Trans.n != length(x)"))
    A = Trans.parent  
    nzval = A.nzval
    colval = A.rowval
    rowptr = A.colptr
